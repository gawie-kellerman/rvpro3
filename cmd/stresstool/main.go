...
func fetchRVProCounters(cfg config.Config, msg string) ([]*hive.RVProRadarStat, error) {
	utils.Print.Ln("Fetching Settings Counters")
	ws := hive.RVProSocket{
...
func runStress(configFilename string, statsFilename string) {
	...
	utils.Print.Ln("Settings Stress Tool - Copyright Radar Vision 2025")
	...
	counters, err := fetchRVProCounters(cfg, "Fetching Startup Settings Counters")
	utils.Debug.Panic(err)
	...
	// Cooldown
	time.Sleep(time.Duration(1) * time.Second)
	counters, err = fetchRVProCounters(cfg, "Fetching Completion Settings Counters")
	utils.Debug.Panic(err)
