...
func fetchRVProCounters(cfg config.Config, msg string) ([]*hive.RVProRadarStat, error) {
	utils.Print.Ln("Fetching Config Counters")
	ws := hive.RVProSocket{
...
func runStress(configFilename string, statsFilename string) {
	...
	utils.Print.Ln("Config Stress Tool - Copyright Radar Vision 2025")
	...
	counters, err := fetchRVProCounters(cfg, "Fetching Startup Config Counters")
	utils.Debug.Panic(err)
	...
	// Cooldown
	time.Sleep(time.Duration(1) * time.Second)
	counters, err = fetchRVProCounters(cfg, "Fetching Completion Config Counters")
	utils.Debug.Panic(err)
