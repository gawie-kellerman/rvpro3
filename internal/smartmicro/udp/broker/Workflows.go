...
func (we *Workflows) GetRadarIP() utils.IP4 {
	return we.RadarIP
}

func (we *Workflows) HasWorkflow(u uint32) bool {
	_, ok := we.Workflows[u]
	return ok
}

func (we *Workflows) GetWorkflow(u uint32) interfaces.IUDPWorkflow {
	res, ok := we.Workflows[u]
	if !ok {
		return nil
	}
	return res
...
func (we *Workflows) Workflow(portIdentifier uint32) interfaces.IUDPWorkflow {
	if we.Workflows == nil {
...
func (we *Workflows) Drop(now time.Time, portIdentifier uint32, bytes []byte) {
	workflow, ok := we.Workflows[portIdentifier]

	if !ok {
		workflow = &Workflow{}
		workflow.Init(we, portIdentifier)
...
func (we *Workflows) onProcess(now time.Time, workflow interfaces.IUDPWorkflow, bytes []byte) {
	startOn := time.Now()
...
func (we *Workflows) onSkip(now time.Time, bytes []byte) {
