...
// UDPDataServiceOld has no IsActive for, if there is
// no KeepAlive then there is no data
type UDPDataServiceOld struct {
	MixinDataService
	ServerIPAddr string
	OnData       func(*UDPDataServiceOld, *net.UDPAddr, []byte)
	buffer       [udpBufferSize]byte
	bufferLen    int
	connection   *net.UDPConn
}

func (s *UDPDataServiceOld) Init() {
...
func (s *UDPDataServiceOld) openConnection() bool {
	var err error
...
func (s *UDPDataServiceOld) receiveData() {
	var err error
...
func (s *UDPDataServiceOld) closeConnection() {
	if s.connection != nil {
