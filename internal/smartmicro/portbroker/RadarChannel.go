...
type RadarChannel struct {
	...
	isDone              bool
	DiagnosticsWorkflow IWorkflow
	ObjectListWorkflow  IWorkflow
	StatisticsWorkflow  IWorkflow
	InstructionWorkflow IWorkflow
	PvrWorkflow         IWorkflow
	TriggersWorkflow    IWorkflow
}

func (rc *RadarChannel) Run(radarIP utils.IP4, workflowBuilder IWorkflowBuilder) {
	...
	rc.DiagnosticsWorkflow = workflowBuilder.GetDiagnosticsWorkflow(radarIP)
	rc.ObjectListWorkflow = workflowBuilder.GetObjectListWorkflow(radarIP)
	rc.StatisticsWorkflow = workflowBuilder.GetStatisticsWorkflow(radarIP)
	rc.InstructionWorkflow = workflowBuilder.GetInstructionWorkflow(radarIP)
	rc.PvrWorkflow = workflowBuilder.GetPVRWorkflow(radarIP)
	rc.TriggersWorkflow = workflowBuilder.GetTriggerWorkflow(radarIP)

	rc.DiagnosticsWorkflow.SetRadarChannel(rc)
	rc.ObjectListWorkflow.SetRadarChannel(rc)
	rc.StatisticsWorkflow.SetRadarChannel(rc)
	rc.InstructionWorkflow.SetRadarChannel(rc)
	rc.PvrWorkflow.SetRadarChannel(rc)
	rc.TriggersWorkflow.SetRadarChannel(rc)
...
func (rc *RadarChannel) process() {
	...
	case port.PiObjectList:
		...
		totalMetricID = int(instrumentation.FmtObjectListTotalTime)
