...
func (f *Font) DrawCh(buffer interfaces.IMonoBuffer, ch uint16, x int, y int) int {
	height := int(f.GetCharHeight())
	width := int(f.GetCharWidth(ch))
	...
	for h := 0; h < height; h++ {
		for w := 0; w < width; w += 8 {
			...
			for iw := 0; iw < min(width-w, 8); iw++ {
				if byteValue&(1<<iw) == 0 {
					buffer.Clear(x+iw+w, h+y)
				} else {
					buffer.Set(x+iw+w, h+y)
				}
			}
		}
	}
...
func (f *Font) DrawStr(buffer interfaces.IMonoBuffer, str string, x int, y int) int {
	runX := x
