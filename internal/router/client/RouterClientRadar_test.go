...
func onWriteSuccess(v *RouterClientRadar, targetIP utils.IP4, dataOnly []byte) {
	fmt.Printf("OnWriteSuccess to %s with %d bytes\n", targetIP, len(dataOnly))
}

func onWriteFail(v *RouterClientRadar, targetIP utils.IP4, dataOnly []byte, err error) {
	fmt.Printf("OnWriteFail to %s with %d bytes, error %v\n", targetIP, len(dataOnly), err)
}

func sendAsRadar(radar *RouterClientRadar, wg *sync.WaitGroup) {
...
func onUDPRead(v *RouterClientRadar, addr utils.IP4, bytes []byte) {
