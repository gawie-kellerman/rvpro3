...
type RouterClientRadar struct {
	...
	OnUDPRead      func(v *RouterClientRadar, addr utils.IP4, bytes []byte)
	OnWriteSuccess func(v *RouterClientRadar, targetIP utils.IP4, dataOnly []byte)
	OnWriteFail    func(v *RouterClientRadar, targetIP utils.IP4, dataOnly []byte, err error)
}
...
type ClientRadarMetrics struct {
	...
	UdpWATBytes               *utils.Metric //  WritePacket After ReadTerminate
...
func (v *RouterClientRadar) Start(ipAddr utils.IP4) {
	...
	sectionName := "Router.RouterClient.Radar.[" + ipAddr.String() + "]"
...
func (v *RouterClientRadar) executeRead() {
...
func (v *RouterClientRadar) executeWrite() {
	for {
		select {
...
func (v *RouterClientRadar) writeData(packetData []byte) {
	...
	if !v.Terminate {
		...
		if targetType == tcphub.PtRadarMulticast {
			...
			//fmt.Println("Writing multicast from ", v.IPAddr, "to", targetAddr.String(), "bytes", pw.GetDataSize(), pw.GetSourceIP4(), pw.GetTargetIP4())
		} else {
...
// to the packet size
func (v *RouterClientRadar) Write(packetData []byte) {
	now := time.Now()
...
func (v *RouterClientRadar) IsTerminated() bool {
	return v.refCount.Load() == 0
}

func (v *RouterClientRadar) onUDPError(
...
func (v *RouterClientRadar) onUDPOpen(connection *utils.UDPServerConnection) {
	v.Metrics.UdpOpen.Inc(1)
}

func (v *RouterClientRadar) onUDPClose(connection *utils.UDPServerConnection) {
